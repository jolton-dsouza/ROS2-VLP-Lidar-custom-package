...
class ROS2Publisher():
    ...
    def timer_callback(self, ros_msg):
        #nodes[self.node_name].get_logger().info('Publishing: "%s %s" \n \n' % (ros_msg.ip, str(nodes[self.node_name +'_ts'])))
        nodes[self.node_name].get_logger().info('Publishing: "%s" at time "%s" \n \n' %(ros_msg.ip, str(nodes[self.node_name].get_clock().now())))
        ...
        if 'radar' in self.node_name.lower(): 
            ...
            while True:
                ...
                for point in message:
                    ...
                    point_struct.timestamp = point[4]
